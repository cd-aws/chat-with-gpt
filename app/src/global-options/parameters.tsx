    ...
    options: [
        {
            ...
            renderProps: (value, options, context) => {
                const description = context.intl.formatMessage(
                    {
                        defaultMessage: "Note: This model will only work if your OpenAI account has been granted access. <a>Request access here.</a>",
                    },
                    { 
                        a: (text) => <a href="https://openai.com/waitlist/gpt-4-api" target="_blank" rel="noreferrer">{text}</a>
                    }
                );
                return {
                    type: "select",
                    label: "Model",
                    description: value?.includes('32') ? description : undefined,
                    options: [
                        {
